...
%% This file is copied from lib/kernel/src/logger_disk_log_h.erl and modified
%% a little bit for automatically truncation
%%
-module(aep_disk_log_h).
...
write(Name, Mode, Bin, State) ->
    case ?disk_log_write(Name, Mode, Bin) of
        {error, {full, LogName}} ->
            io:format("~s truncate logger [~p] as disk full, trunc result: [~p], in [~p] mode~n", [calendar:system_time_to_rfc3339(erlang:system_time(millisecond),
                    [{unit, millisecond}, {time_designator, $\s}]),
             LogName, aep_disk_log:btruncate(Name, <<>>), Mode]),
            %% try to write the failure msg one more time
            Result = ?disk_log_write(Name, async, Bin),
...
%% The disk log owner must handle status messages from aep_disk_log.
handle_info(Name, {aep_disk_log, _Node, Log, Info={truncated,_NoLostItems}}, State) ->
    maybe_notify_status(Name, Log, Info, prev_disk_log_info, State);
handle_info(Name, {aep_disk_log, _Node, Log, Info = {blocked_log,_Items}}, State) ->
    maybe_notify_status(Name, Log, Info, prev_disk_log_info, State);
handle_info(Name, {aep_disk_log, _Node, Log, Info = full}, State) ->
    maybe_notify_status(Name, Log, Info, prev_disk_log_info, State);
handle_info(Name, {aep_disk_log, _Node, Log, Info = {error_status,_Status}}, State) ->
    maybe_notify_status(Name, Log, Info, prev_disk_log_info, State);
    ...
    case filelib:ensure_dir(File) of
        ok ->
            ...
            Opts = [{name,   Name},
                    ...
                    {quiet,  true},
                    {mode,   read_write}],
            case aep_disk_log:open(Opts) of
                {ok,Name} ->
                    ok;
                Error = {error,_Reason} ->
                    Error
...
close_disk_log(Name, _) ->
    ...
    _ = aep_disk_log:lclose(Name),
    ok.

disk_log_write(Name, sync, Bin) ->
    aep_disk_log:blog(Name, Bin);
disk_log_write(Name, async, Bin) ->
    aep_disk_log:balog(Name, Bin).
...
maybe_notify_status(Name, Log, Info, Key, State) ->
    error_notify_new({aep_disk_log, Name, Log, Info}, Info, Key, State).
