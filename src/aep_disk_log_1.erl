...
repair(In, File) ->
    ...
    case is_quiet() of
        true -> ok;
        _ -> error_logger:info_msg("aep_disk_log: repairing ~tp ...\n", [File])
    end,
...
mf_int_chunk(Handle, {FileNo, Pos}, Bin, N) ->
	...
	{error, _Reason} ->
	    case is_quiet() of
		true -> ok;
		_ -> error_logger:info_msg("aep_disk_log: chunk error. File ~tp missing.\n\n",
					   [FName])
	    end,
