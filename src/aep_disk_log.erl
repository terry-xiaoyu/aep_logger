...
-spec open(ArgL) -> open_ret() | dist_open_ret() when
      ArgL :: dlog_options().
open(A) ->
    aep_disk_log_server:open(check_arg(A, #arg{options = A})).
...
-spec accessible_logs() -> {[LocalLog], [DistributedLog]} when
      ...
      DistributedLog :: log().
accessible_logs() ->
    aep_disk_log_server:accessible_logs().

istart_link(Server) ->
    {ok, proc_lib:spawn_link(aep_disk_log, init, [self(), Server])}.
...
check_arg([], Res) ->
	...
	Res#arg.type =:= wrap ->
	    {OldSize, Version} = 
		aep_disk_log_1:read_size_file_version(Res#arg.file),
...
do_exit(S, From, Message0, Reason) ->
    ...
    _ = aep_disk_log_server:close(self()),
...
do_fast_exit(S, Server, Message) ->
    _ = do_stop(S),
    Server ! {aep_disk_log, self(), Message},
    exit(normal).
...
proc_q([{From, _R}|Tail]) when is_pid(From) ->
    From ! {aep_disk_log, self(), {error, disk_log_stopped}},
    proc_q(Tail);
...
do_open(A) ->
    ...
    aep_disk_log_1:set_quiet(Quiet),
...
do_change_size(#log{type = wrap}=L, NewSize) ->
    ...
    {ok, Handle} = aep_disk_log_1:change_size_wrap(Extra, NewSize, Version),
    erase(is_full),
...
do_inc_wrap_file(L) ->
    ...
    case Format of
	internal ->
	    case aep_disk_log_1:mf_int_inc(Handle, L#log.head) of
		{ok, Handle2, Lost} ->
	...
	external ->
	    case aep_disk_log_1:mf_ext_inc(Handle, L#log.head) of
		{ok, Handle2, Lost} ->
		    {ok, L#log{extra = Handle2}, Lost};
		{error, Error, Handle2} ->
		    {error, Error, L#log{extra = Handle2}}
...
do_open2(halt, internal, Name, FName, Repair, Size, Mode, Head, _V) ->
    case catch aep_disk_log_1:int_open(FName, Repair, Mode, Head) of
...
do_open2(wrap, internal, Name, FName, Repair, Size, Mode, Head, V) ->
    {MaxB, MaxF} = Size,
    case catch 
      aep_disk_log_1:mf_int_open(FName, MaxB, MaxF, Repair, Mode, Head, V) of
	{ok, Handle, Cnt} ->
...
do_open2(halt, external, Name, FName, Repair, Size, Mode, Head, _V) ->
    case catch aep_disk_log_1:ext_open(FName, Repair, Mode, Head) of
	{ok, {_Alloc, FdC, {NoItems, _NoBytes}, FileSize}} ->
            Halt = #halt{fdc = FdC, curB = FileSize, size = Size},
...
do_open2(wrap, external, Name, FName, Repair, Size, Mode, Head, V) ->
    {MaxB, MaxF} = Size,
    case catch 
      aep_disk_log_1:mf_ext_open(FName, MaxB, MaxF, Repair, Mode, Head, V) of
	{ok, Handle, Cnt} ->
...
close_disk_log2(L) ->
	...
	#log{format_type = halt_int, mode = Mode, extra = Halt} ->
	    aep_disk_log_1:close(Halt#halt.fdc, L#log.filename, Mode);
	#log{format_type = wrap_int, mode = Mode, extra = Handle} ->
	    aep_disk_log_1:mf_int_close(Handle, Mode);
	#log{format_type = halt_ext, extra = Halt} ->
	    aep_disk_log_1:fclose(Halt#halt.fdc, L#log.filename);
	#log{format_type = wrap_ext, mode = Mode, extra = Handle} ->
	    aep_disk_log_1:mf_ext_close(Handle, Mode)
    end,
    closed.
...
do_info(L, Cnt) ->
    ...
    Size = case Type of
	       wrap ->
		   aep_disk_log_1:get_wrap_size(Extra);
	       halt ->
		   Extra#halt.size
	   end,
    Distribution =
	case aep_disk_log_server:get_log_pids(Name) of
	    {local, _Pid} -> 
		local;
	    {distributed, Pids} ->
                [node(P) || P <- Pids];
...
do_log(#log{format_type = wrap_int}=L, B, _BSz) ->
    case aep_disk_log_1:mf_int_log(L#log.extra, B, L#log.head) of
...
do_log(#log{format_type = wrap_ext}=L, B, _BSz) ->
    case aep_disk_log_1:mf_ext_log(L#log.extra, B, L#log.head) of
	{ok, Handle, Logged, Lost, Wraps} ->
	    notify_owners_wrap(Wraps),
	    put(log, L#log{extra = Handle}),
	    Logged - Lost;
...
halt_write(Halt, L, B, Bs, BSize) ->
    case aep_disk_log_1:fwrite(Halt#halt.fdc, L#log.filename, Bs, BSize) of
	{ok, NewFdC} ->
...
do_write_cache(#log{filename = FName, type = halt, extra = Halt} = Log) ->
    {Reply, NewFdC} = aep_disk_log_1:write_cache(Halt#halt.fdc, FName),
    put(log, Log#log{extra = Halt#halt{fdc = NewFdC}}),
    Reply;
do_write_cache(#log{type = wrap, extra = Handle} = Log) ->
    {Reply, NewHandle} = aep_disk_log_1:mf_write_cache(Handle),
    put(log, Log#log{extra = NewHandle}),
    Reply.

%% -> ok | Error
do_sync(#log{filename = FName, type = halt, extra = Halt} = Log) ->
    {Reply, NewFdC} = aep_disk_log_1:sync(Halt#halt.fdc, FName),
    put(log, Log#log{extra = Halt#halt{fdc = NewFdC}}),
    Reply;
do_sync(#log{type = wrap, extra = Handle} = Log) ->
    {Reply, NewHandle} = aep_disk_log_1:mf_sync(Handle),
    put(log, Log#log{extra = NewHandle}),
    Reply.
...
do_trunc(#log{type = halt}=L, Head) ->
    ...
    {Reply1, FdC2} = 
        case L#log.format of
            internal ->
                aep_disk_log_1:truncate(FdC, FName, Head);
            external ->
		case aep_disk_log_1:truncate_at(FdC, FName, bof) of
		    {ok, NFdC} when Head =:= none ->
                        {ok, NFdC};
		    {ok, NFdC} ->
			{ok, H} = Head,
                        aep_disk_log_1:fwrite(NFdC, FName, H, byte_size(H));
		    R -> 
			R
		end
        end,
    {Reply, NewHalt} = 
    case aep_disk_log_1:position(FdC2, FName, cur) of
...
do_trunc(#log{type = wrap}=L, Head) ->
    Handle = L#log.extra,
    OldHead = L#log.head,
    {MaxB, MaxF} = aep_disk_log_1:get_wrap_size(Handle),
...
do_chunk(#log{format_type = halt_int, extra = Halt} = L, Pos, B, N) ->
    ...
    {NewFdC, Reply} = 
        case L#log.mode of
            read_only ->
                aep_disk_log_1:chunk_read_only(FdC, L#log.filename, Pos, B, N);
            read_write ->
                aep_disk_log_1:chunk(FdC, L#log.filename, Pos, B, N)
        end,
    put(log, L#log{extra = Halt#halt{fdc = NewFdC}}),
    Reply;
do_chunk(#log{format_type = wrap_int, mode = read_only, 
	      extra = Handle} = Log, Pos, B, N) ->
    {NewHandle, Reply} = aep_disk_log_1:mf_int_chunk_read_only(Handle, Pos, B, N),
    put(log, Log#log{extra = NewHandle}),
    Reply;
do_chunk(#log{format_type = wrap_int, extra = Handle} = Log, Pos, B, N) ->
    {NewHandle, Reply} = aep_disk_log_1:mf_int_chunk(Handle, Pos, B, N),
    put(log, Log#log{extra = NewHandle}),
    Reply;
...
do_chunk_step(#log{format_type = wrap_int, extra = Handle}, Pos, N) ->
    aep_disk_log_1:mf_int_chunk_step(Handle, Pos, N);
do_chunk_step(Log, _Pos, _N) ->
    {error, {not_internal_wrap, Log#log.name}}.

%% Inlined.
replies(Pids, Reply) ->
    M = {aep_disk_log, self(), Reply},
    send_reply(Pids, M).
...
reply(To, Reply, S) ->
    To ! {aep_disk_log, self(), Reply},
    loop(S).

req(Log, R) ->
    case aep_disk_log_server:get_log_pids(Log) of
	{local, Pid} ->
...
monitor_request(Pid, Req) ->
	...
	{'DOWN', Ref, process, Pid, _Info} ->
	    {error, no_such_log};
	{aep_disk_log, Pid, Reply} when not is_tuple(Reply) orelse
...
wrap_file_extensions(File) -> 
	...
	aep_disk_log_1:read_index_file(File),
    Fs = if 
	     NoOfFiles >= 1 ->
...
notify_owners(Note) ->
    L = get(log),
    Msg = {aep_disk_log, node(), L#log.name, Note},
