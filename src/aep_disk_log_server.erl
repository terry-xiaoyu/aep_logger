...
%%%-----------------------------------------------------------------
%%% This module implements the aep_disk_log server.  Its primary purpose
...
%%%----------------------------------------------------------------------
start_link() ->  
    gen_server:start_link({local, aep_disk_log_server}, aep_disk_log_server, [], []).
...
open({ok, A}) ->
    ensure_started(),
    gen_server:call(aep_disk_log_server, {open, local, A}, infinity);
open(Other) ->
    Other.
...
dist_open(A) ->
    ensure_started(),
    gen_server:call(aep_disk_log_server, {open, distr, A}, infinity).

close(Pid) ->
    gen_server:call(aep_disk_log_server, {close, Pid}, infinity).
...
handle_info({pending_reply, Pid, Result0}, State) ->
    ...
    if 
        Attach and (Result0 =:= {error, no_such_log}) ->
            %% The aep_disk_log process has terminated. Try again.
...
ensure_started() ->
    case whereis(aep_disk_log_server) of
	undefined ->
	    ...
	    LogServer = {aep_disk_log_server,
			 {aep_disk_log_server, start_link, []},
			 permanent, 2000, worker, [aep_disk_log_server]},
...
do_internal_open(Name, Pid, From, {open, _W, A}=Req, Attach, State) ->
    Server = self(), 
    F = fun() -> 
                Res = aep_disk_log:internal_open(Pid, A),
...
do_get_log_pids(LogName) ->
	...
	[{LogName, _Pid, distr}] ->
            case pg2:get_members(?group(LogName)) of
                [] -> % The aep_disk_log process has died recently
                    undefined;
                Members -> 
                    {distributed, Members}
            end;
...
dist_pids(LogName) ->
	...
	[Pid | _] = Pids -> 
            case rpc:call(node(Pid), ?MODULE, get_local_pid, [LogName]) of
                undefined -> % does not seem to be a aep_disk_log group
